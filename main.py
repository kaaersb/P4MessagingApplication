...
@app.get("/me")
def whoami(session: dict = Depends(require_session)):
    """
    Return the currently authenticated user, based solely on the session
    cookie. Used by the frontend on page load to restore an existing session
    so a refresh does not force the user to log in again.
    Returns 401 (via require_session) when there is no valid session.
    """
