...
// Attempt to restore an existing session on page load. The session cookie is
// HttpOnly, so we can't read it from JS — instead we ask the server who we are
// via GET /me, which succeeds only if a valid session cookie was sent. On
// success we go straight into the app; on failure we fall back to the welcome
// screen. The auth screen is hidden up front to avoid a flash of the login
// view while the check is in flight.
async function restoreSession() {
    ...
    try {
        ...
        // No valid session — show the normal welcome screen.
...
async function handleLogout() {
    ...
    // Reset to the Friends tab so the next login starts in a known state.
...
function startFallbackPoll() {
    ...
    state.fallbackPollInterval = setInterval(async () => {
        ...
        // Refresh sidebar (friends + pending requests) every ~9s rather than
        // every 30s, so an incoming friend request surfaces promptly even when
        // the WebSocket push is missed. The WS remains the primary, instant path.
        if (tick % 3 === 0) {
            await loadFriends(true);
...
async function loadFriends(silent = false) {
    try {
        ...
        // Repaint the visible list regardless of which tab is active.
        // renderContactList() chooses the friends/requests view internally
        // based on state.currentTab, so an update is never silently dropped
        // just because the user happens to be on the other tab.
...
async function loadPendingRequests(silent = false) {
    try {
        ...
        // Repaint regardless of active tab (see note in loadFriends).
        renderContactList(document.getElementById('contact-search').value);
    } catch (err) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ── Restore an existing session (skips the login screen on refresh) ──
    restoreSession();
});
